...
- [18 — Figure da letteratura](18-literature-figures.md): figure di fonte estratte dai documenti del corso (Docling in un worker con processo figlio, Vision descrittiva), catalogo e budget separato nel prompt di Fase 3, riga «Fonte» calcolata a render su dispensa, slide, frame video, vista ed editor, ciclo di vita non retroattivo, letteratura aperta (Wikimedia Commons, OpenAlex) per le lezioni in buco, formato `tikz` compilato da XeLaTeX in sandbox (spento di default); misure, decisioni, deviazioni e limiti.
...
│   ├── figure_render_service.py             # Figure — registro dei renderer (Protocol FigureRenderer, render_figure_map e la proiezione render_svg_map, validate_visual_assets_or_raise; WP2b, WP3)
