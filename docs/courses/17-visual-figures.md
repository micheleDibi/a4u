...
SEC-1) le **direttive di shape `@{ … }`** dei flowchart 11 che contengono
`img:` o un URL (`http(s)`, `file`, `data`, `blob`, `ftp`): la shape
immagine diventa un `<image href="…">` che il Chromium del pre-render,
