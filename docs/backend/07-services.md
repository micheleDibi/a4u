...
| Vision e costo | `openai_figure_describe_service.py` (PROMPT 18), `openai_http.py`, `scripts/redescribe_figure_depicts.py` | Descrizione, tipo, parole chiave e `depicts` (oggetti e varianti raffigurati); trasporto OpenAI condiviso con retry (`post_chat_with_retry`); completamento di `depicts` sulle figure già descritte |
...
| Abbinamento e assegnazione | `figure_need_matching.py`, `source_figure_assignment.py` (puri), `source_figure_assignment_service.py` | Relazione fabbisogno ↔ figura su `depicts`, assegnazione globale con tetto di riuso e budget del piano, lock di corso, offerta e fotografia in `figure_assignment` |
| Blocco del piano ed editor | `source_figure_plan.py`, `figure_needs_view.py` (puri) | Catalogo del piano per il PROMPT 3 (per sezione e fabbisogno), collocazione e taglio per priorità dopo la fusione; vista dei fabbisogni per l'editor calcolata alla lettura (stati, conteggi, sequenze per i PROMPT 5 e 6) |
| Figure inserite dal sistema | `source_figure_fill.py`, `openai_figure_intro_service.py` (PROMPT 23) | Completamento dopo la generazione (lezioni pronte e non approvate, sotto il lock di corso), candidati e «Inserisci» dell'editor, frase che introduce ogni figura inserita |
