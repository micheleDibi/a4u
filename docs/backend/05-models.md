...
- `Course` ha uno status-**indicatore** a 22 valori che copre l'intera pipeline: `draft → architecture_* → lessons_structure_* → content_* → slides_* → speech_* → video_* → avatar_video_* → published / archived` (rank monotòno in `core/course_phase_order.COURSE_STATUS_RANK`; col gating per-unità è una milestone "fase massima raggiunta", non un lock). Nota: la tupla `COURSE_STATUSES` in `models/course.py` e il CHECK inline `ck_course_status_valid` elencano ora **tutti i 22** valori (drift sanato: il modello ne dichiarava 18 — mancavano `video_pending/ready` e `avatar_video_pending/ready`, che in DB c'erano già dalla migration `0030`; video/avatar non hanno `approved`). Ogni `*_pending/_ready/_approved` è derivato dagli stati per-modulo (Fase 2) o per-lezione (Fasi 3-6b). Ha inoltre `video_language_code` (VARCHAR(10) nullable, FK `languages.code` ON DELETE SET NULL, migration `0026`): override opzionale della lingua della voce TTS nei video MP4 (Fase 6); NULL → fallback su `language_code`. La relationship `video_language` richiede `foreign_keys=` esplicito perché due FK puntano a `languages.code`. Il campo `corso_di_laurea` (VARCHAR(200) nullable, migration `0033`) è il nome libero del corso di laurea (es. "Informatica", "Ingegneria Gestionale"), mostrato dal FE solo quando il livello EQF selezionato è Laurea triennale (`eqf_6_bachelor`) o Laurea Magistrale (`eqf_7_master_degree`).
- `CourseDocument` ha il proprio state machine `pending → processing → ready/failed` per il pre-processing AI (Appendice A).
- Figure da letteratura (migrazioni `0037`/`0038`, [Courses 18 § 3](../courses/18-literature-figures.md)): `CourseDocument` ha in più la provenienza (`origin`, `is_own_work`, `license`, `bibliography*`) e lo stato dell'estrazione figure, separato dal riassunto (`figures_status`, NULL = mai chiesta). La nuova `CourseDocumentFigure` (`app/models/course_document_figure.py`) è il catalogo per corso: FK al documento `ON DELETE SET NULL` con `detached_at`, `source_kind` `uploaded|openalex|wikimedia`, `license` NOT NULL senza default, `attribution` congelata, dati Vision e `vision_usage`. `CourseLesson` ha `content_figure_review` (verdetti del revisore, fuori da `content_raw`) e lo stato dei buchi `figures_gap_*`. `OrganizationCourseSettings` ha `figure_source_license_policy`. Con la migrazione `0039` ([Courses 18 § 22](../courses/18-literature-figures.md)) le figure hanno gli ingressi della risoluzione effettiva (`native_ppi`, `natural_width_mm`, `crop_mode`, `crop_version`) e del ri-ritaglio reversibile (`recropped_at`, `recrop_previous`); `CourseDocument` ha `figures_recrop_requested_at` e `figures_recrop_stats`. Con la `0040` ([Courses 18 § 23](../courses/18-literature-figures.md)) `CourseLesson` ha i fabbisogni del piano delle figure (`figure_needs` con impronta e `need_id` stabili, stato `figure_needs_status` con CHECK, `figure_needs_attempts`, `figure_needs_requested_at`, `figure_needs_checked_at`, costo cumulativo `figure_needs_usage`). Con la `0041` `CourseDocumentFigure` ha `depicts` (oggetti e varianti raffigurati in inglese canonico, scritto dai PROMPT 18 e 20). Con la `0042` `CourseLesson` ha `figure_assignment` (offerta e fotografia dell'assegnazione) e `CourseDocumentFigure` ha `found_for_lesson_id`/`found_for_need_id` (letteratura trovata per un fabbisogno). Con la `0043` `CourseLesson` ha `figure_need_links` (collegamenti del docente ai fabbisogni: «Non serve» o figura collegata) e due campi calcolati alla lettura, `figure_needs_view` e `figure_needs_summary` (`services/figure_needs_view.py`).
...
- `CourseLesson` è la tabella più ampia: oltre ai campi base e Fase 2 (`learning_objectives`, `mandatory_topics`, `prerequisites`, `section_outline`), ha questi blocchi di colonne per le pipeline AI/PDF/video:
  ...
  I due index video/avatar (già in DB via migration 0025/0029) sono ora **dichiarati anche in `course_lesson.py`** (`__table_args__`), così gli ambienti `create_all` (test) hanno lo stesso schema.
