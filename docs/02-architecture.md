...
- **2** — i tre worker PDF (testo / slide / discorso) — condividono `COURSE_LESSON_PDF_MAX_CONCURRENCY` perché tutti CPU-bound su WeasyPrint; il pre-render delle figure (Mermaid via Chromium, Vega-Lite e sympy in un processo figlio `spawn`, `dot` in subprocess, matplotlib in thread) passa dal semaforo separato `FIGURE_RENDER_MAX_WORKERS` (2) di `figure_render_service.render_figure_map` (di cui `render_svg_map` è la proiezione `.svg`), condiviso con le anteprime `render-function` dell'editor (vedi [Courses 17](courses/17-visual-figures.md))
