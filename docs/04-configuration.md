...
| `FIGURE_LITERATURE_MAX_CANDIDATES_PER_LESSON` | `15` | Candidate valutate dalla Vision (PROMPT 20) per lezione (era 8; con il piano delle figure la ricerca è per fabbisogno). Vale anche senza piano, con i tetti in dollari e di PDF qui sotto. |
...
| `FIGURE_LITERATURE_MAX_COST_USD_PER_CHECK` | `0.08` | Tetto in dollari di una verifica, retry compresi (Vision e copie OpenAlex); ci si ferma prima della Vision che lo supererebbe. |
| `FIGURE_LITERATURE_MAX_PAID_PDF_PER_LESSON` | `3` | Copie OpenAlex a pagamento (0,01 $) per verifica; una copia già pagata nel giro non si ripaga. |
...
| `FIGURE_LITERATURE_MAX_IMAGE_PIXELS` | `16000000` | Pixel massimi di un'immagine, controllati prima di decodificarla (le miniature di Commons sono larghe 1920 px; 16 MP tengono bassi i picchi di memoria). |
...
| `FIGURE_PLAN_IN_PROMPT_ENABLED` | `true` | Blocco del piano delle figure nel messaggio user del PROMPT 3 (figure assegnate per sezione e fabbisogno, doc 18 §23.6). Spento: nessuna offerta e catalogo lessicale come prima; fabbisogni e verifica dei buchi restano. |
| `FIGURE_AUTO_FILL_ENABLED` | `true` | Completamento automatico: una figura adatta trovata dopo la generazione (letteratura aperta, estrazione dei documenti) entra da sola nelle lezioni non approvate, nella sezione del suo fabbisogno, con la frase che la introduce (doc 18 §24). |
| `FIGURE_INTRO_SENTENCE_ENABLED` | `true` | Frase che introduce le figure inserite dal sistema (PROMPT 23). Spento: frase fissa dalla didascalia (italiano e inglese), nessuna chiamata. |
| `OPENAI_FIGURE_INTRO_MODEL` | `gpt-4.1-mini` | Modello del PROMPT 23 (solo testo). |
| `OPENAI_FIGURE_INTRO_REASONING_EFFORT` | _(vuoto)_ | Solo per modelli reasoning. |
| `OPENAI_FIGURE_INTRO_MAX_TOKENS` | `400` | Tetto dell'output del PROMPT 23. |
