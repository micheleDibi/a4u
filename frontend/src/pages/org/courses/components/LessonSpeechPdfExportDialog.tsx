...
interface Props {
  ...
  /** Etichetta della lezione singola. */
  lessonLabel?: string;
...
/**
 * Dialog di export PDF del discorso (Fase 5 §8). Mirror di
 * `LessonPdfExportDialog` perché entrambi usano `pdf_templates`
 * (kind=lesson). Distinto dal `LessonSlidesPdfExportDialog` che usa
 * `slide_templates`.
 */
export function LessonSpeechPdfExportDialog({
  open,
  mode,
  exportableCount,
  lessonLabel,
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-3 py-2">
          <Label className="text-sm font-medium">
            {t("courses.lessonsSpeechPdf.dialog.templateLabel")}
          </Label>
          ...
          {!tplQuery.isLoading && (tplQuery.data?.length ?? 0) === 0 && (
            <div className="rounded-md border border-dashed border-muted-foreground/30 bg-muted/30 px-4 py-6 text-center text-sm text-muted-foreground">
              {t("courses.lessonsSpeechPdf.dialog.noTemplates")}
            </div>
          )}
        ...
        <div className="flex items-center gap-2">
          ...
          {tpl.is_default && (
            <Badge variant="secondary" className="gap-1 text-[11px]">
              ...
              {t("courses.lessonsSpeechPdf.dialog.defaultBadge")}
            </Badge>
          )}
        </div>
