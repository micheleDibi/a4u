...
export default function PdfTemplatesListPage() {
  ...
  const remove = useMutation({
    mutationFn: (id: string) => pdfTemplatesApi.remove(orgId, id),
    onSuccess: () => {
      toast.success(t("templates.pdf.deleted"));
  ...
  const setDefault = useMutation({
    mutationFn: (id: string) => pdfTemplatesApi.setDefault(orgId, id),
    onSuccess: () => {
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {(query.data ?? []).map((tpl) => (
          <Card key={tpl.id} className="group relative overflow-hidden">
            ...
            <button
              type="button"
              onClick={() => navigate(`/orgs/${orgId}/templates/pdf/${tpl.id}`)}
            ...
            >
              <div className="mx-auto max-w-[280px]">
                <PdfTemplatePreview
                  ...
                  headerHeightMm={tpl.header_height_mm}
                  footerHeightMm={tpl.footer_height_mm}
                  marginMm={tpl.margin_mm}
                  backgroundOpacityPct={tpl.background_opacity_pct}
                />
              </div>
            ...
            <CardContent className="flex items-start justify-between p-4 pt-0">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem
                    onSelect={() => navigate(`/orgs/${orgId}/templates/pdf/${tpl.id}`)}
                  >
